...
export class VersionRepository {
  ...
  async create(scene: LiveScene, name: string): Promise<ProjectVersion> {
    ...
    const version: ProjectVersion = {
      ...
      snapshot: Serializer.capture(scene.objects, scene.materials, scene.assembly, scene.assets, scene.referenceImages),
    };
  ...
  async restore(scene: LiveScene, version: ProjectVersion): Promise<void> {
    Serializer.apply(scene.objects, scene.materials, scene.assembly, scene.assets, scene.referenceImages, version.snapshot);
